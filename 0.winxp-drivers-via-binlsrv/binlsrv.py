...
BASEPATH = '/tftproot/1.winxp-x86-ch'
WELCOME  = 'welcome.osc'
DUMPING  = False
...
## add the rev filed (by blueardour)
def send_ncr(s, addr, vid, pid, subsys, rev):
    ...
    device_sub_rev = device_sub + '&REV_%02X' % rev

    dev = None
    try:
        print 'Checking', device_sub_rev
        dev = devlist[device_sub_rev]
        dev_uni = device_sub_rev
    except:
        try:
